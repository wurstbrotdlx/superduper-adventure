...
| **GitHub-Pages-Live-Check — wichtiger Befund** | Über die echte `https://wurstbrotdlx.github.io/…`-URL zeigt der Sandbox-Browser dieser Sitzung massenhaft „Sprite fehlt"-Warnungen (praktisch jedes Sheet ab `Angel_1.png` in der Ladeliste) — reproduzierbar in zwei frischen Tabs. **Dieselbe Datei (`dist/index.html`, byte-identisch zu `docs/index.html`), lokal per `python3 -m http.server` bedient, lädt fehlerfrei.** Die einzige Variable ist „externe HTTPS-Origin im Sandbox-Browser" vs. „localhost" — das deutet auf eine Eigenheit des Browser-Tools dieser Sitzung beim Laden einer großen, fast nur aus `data:`-URIs bestehenden Seite von einem echten Fremd-Host hin, nicht auf einen Fehler im Build oder im Code. **Nicht abschließend im Sandbox-Browser verifizierbar — Matthias bitte selbst `https://wurstbrotdlx.github.io/superduper-adventure/` in einem normalen Browser öffnen und gegenchecken**, bevor das als „live verifiziert" gilt. |
