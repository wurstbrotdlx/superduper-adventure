...
Gemeint war die T3-Auflage, die seit zwei Abschnitten über den drei
Szenen-Anlässen steht: *jede Zeile muss aus dem Haus kommen, nicht aus der
Akte.* Die Antwort auf die Frage lautet: die Auflage ist nicht zu streng, sie
ist nur nicht der Grund. Zwei andere Regeln waren es.
...
Bewegungsreihe. Dort ist er richtig, denn wer zieht, ist immer der Spieler.
Als allgemeine Tonregel gelesen war er zu weit gefasst und hat das Falsche
...
Tipp, der stehen bliebe, wäre eine Behauptung über den Fall, und genau die
darf sie nicht haben. Der Guard zählt die Rate-Paare und verlangt mindestens
...
Die Prüfung hat sich beim ersten Lauf sofort bezahlt gemacht: genau diese
...
Sache, die überraschen soll, und das Haus hat dazu seit T3 einen Satz: die
einzige Ankündigung, die Anlage 2 im ganzen Spiel bekommt, ist die vierzehnte
Zeile der Ernennungsurkunde, und wer sie überliest, macht alles richtig.

Der bestehende Punkt „Zwei Stimmen statt einer" beschreibt den Kanal weiterhin
...
## 4. Ein Fund, und er ist älter als dieser Bauabschnitt
...
`t.indexOf(w)`. Eine Quelle ohne Text hat den Guard damit **abstürzen**
lassen, statt ihn melden zu lassen, und weil er auf Skriptebene läuft, nahm er
das halbe Spiel mit: die Probe „Ausbruch ohne Rücknahme" ließ das Spiel gar
nicht mehr anlaufen.
...
**Und ein Fund an der Probe selbst.** Die erste Fassung des Probenskripts
horchte nur auf `console.error`. Ein nicht abgefangener `TypeError` kommt als
...
| `tools/reich-pruef.mjs` | 59 von 59 | 59 von 59 |
| `tools/stopfen-pruef.mjs` | 43 von 43 | 43 von 43 |
| `tools/versuchung-pruef.mjs` | 67 von 67 | 67 von 67 |
| `tools/langvorgang-pruef.mjs` | 58 von 58 | 58 von 58 |
| `tools/zulagen-pruef.mjs` | 50 von 50 | 50 von 50 |
| `tools/mitteilung-pruef.mjs` | 32 von 32 | 32 von 32 |
| `tools/gespraech-pruef.mjs` | 87 von 89 | 87 von 89, siehe unten |
| `tools/ebene-pruef.mjs` | 53 von 54 | 53 von 54, siehe unten |
...
**Der Umschlag hat noch einen Platz.** Dreizehn Zeilen, der Guard lässt bis
vierzehn zu. Unverändert seit der T4-Nachlese.
