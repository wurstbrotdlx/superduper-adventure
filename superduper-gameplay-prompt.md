...
* `let kammer` und `let owSave` stehen **ganz oben** bei den Kachel-Konstanten, nicht im Kammerblock: `computeTile()` läuft schon beim Kartenbau und fragt `kammer` ab. Weiter unten deklariert wäre es ein TDZ-Fehler, genau wie bei den Tabellen aus Phase 1.
...
* Geometrie: `KAM_W`/`KAM_H` (13 x 15 Kacheln je Raum, Wände eingerechnet), Korridor ab `KAM_X0`/`KAM_Y0`, alle Tore in Zeile `KAM_TY` (Raummitte). Maximal 6 Räume, also 75 Kacheln Breite. Die Ecke liegt bewusst unabhängig von der Oberwelt-Anordnung, weil `betreteKammer()` die Karte ohnehin komplett überschreibt. **Modul-Layouts rechnen relativ zu `r.y0`** — wer `KAM_H` ändert, muss die Offsets in den `bau*`-Funktionen mitziehen.
* `kammerKamera()` hält den Korridor mittig, sonst zeigt die Kamera an den Rändern nur Leere.
* Tore machen beim Öffnen **nur die Chunks ihrer Umgebung** ungültig (`invalidateMoore`), sie zeichnen nichts nach. Bis W-Groß gab es dafür `malBoden()`, weil ein Komplettbake des damaligen Vollkarten-Canvas 6400 `drawImage` mitten im Spiel bedeutet hätte; mit dem Chunk-Cache erledigt sich das. Die volle Moore-Nachbarschaft ist Pflicht, nicht Vorsicht: `dunWallTile()` liest für die 9-Slice-Kante auch die Diagonalen.
...
* Übergabe zwischen zwei Schichten lief bis SP2 über die Laufzeitvariablen `pendingCarryGold` und `pendingCarryPouch`, die `startGame()` bei jedem Seitenaufruf leerte. **Seit SP2 ist es `amt.uebertrag`** (`{gold, zutaten}` oder `null`): `endShift()` schreibt den schon gekappten Übertrag in die Akte, `startShift()` löst ihn ein und leert ihn im selben Zug, Bauform wie `amt.wiedereinsetzung`. Grund war ein Verlust, kein Aufräumen — wer im Dorf zwischen Dienstbericht und Antritt schloss, verlor Gürtelgold und Kontingent, obwohl der Bericht „mitgenommen" druckte.
...
**Beat 3 achtet auf `player.spellPoints`, nicht auf `player.skillPoints`.** Ein Level-Up vergibt beides (`player.skillPoints += 2; player.spellPoints += 1;`, ~1644). `T` öffnet den Zauberbaum, der die Zauberpunkte verbraucht; die Skillpunkte sitzen im Inventar. **Seit U8 überholt:** die Befähigung steht im **Charakterfenster** (Taste `C`), nicht mehr im Rucksack — was am Träger hängt, steht beim Träger (`phase-u8-menuefenster.md`). Der Zauberbaum bleibt sein eigenes Fenster auf `T`.
...
* **Scheduler**. Lookahead-Muster (25 ms Tick, 120 ms Horizont), plant über `AC.currentTime`, nicht über `setTimeout`-Wartezeiten. Das ersetzt den alten Takt, der über die Zeit hörbar driftete. Zonenwechsel werden nur an Taktgrenzen angewendet (`stepIdx % z._stepsPerBar === 0`), nie mitten im Takt. Bewusst der Takt und nicht die Viertakt-Phrase (R7/F35: die stand vorher im Code und ließ die Kammermusik bis zu 12,6s nach dem Verlassen weiterlaufen). Die neue Zone setzt weiterhin am Kopf ihres Arrangements ein, verkürzt wird nur die abgehende.
* **`MUS`**, die einzige nach außen sichtbare Schnittstelle, sechs Methoden: `goto(zone)`, `layer(name, on)`, `sting(name)`, `swell()`, `muffle(on)`, `setOvertime(f)`. Das Ducken gehört nicht dazu: `duckToFx(ms, depth)` ist audiointern und wird nur aus dem Audioblock ausgelöst, beim Zonenwechsel und am Ende von `sting()`. Ein `duck(ms)` stand hier bis R7/F21, ohne dass es je gebaut oder abgenommen wurde (die Abnahmezeile unten zählt es nicht mit auf).
