...
* **F19 Gold-Doppelbuchung. NEU ENTSCHIEDEN am 2026-08-05 durch Matthias: keine Doppelbuchung. Umgesetzt, entgegen dem Absatzkopf ausnahmsweise mit Codeeingriff.** `endShift()` verbucht nicht mehr denselben `carryGold` zweimal, sondern schreibt der Amtskasse den einbehaltenen Rest gut (`const abgabe = player.gold - carryGold; amt.bankGold += abgabe`). Die Summe beider Buchungen ist jetzt exakt `player.gold`. Die Berichtszeile heißt nicht mehr „Beuteanteil nach Abzug: 💰 X (von Y)", sondern nennt beide Hälften: „Beuteaufkommen: 💰 Y (X mitgenommen, Z an die Amtskasse)" — nach dem Muster der Zutatenzeile darüber. Damit ist auch die Beschriftungsfrage erledigt, die hier vorher an F20 gehängt war.
  **Überholt am 20.08.2026:** „100 Prozent überleben" gilt nicht mehr. Der Verwaltungskostenanteil ist gebaut (`goldAufteilung()`, `CONFIG.verwaltungskosten = 0.2`), die Schichtbeute geht zu 50 Prozent an den Gürtel, zu 30 an die Amtskasse und zu 20 verloren. Damit ist auch die unten genannte Entkopplung erledigt: der Bankzufluss hängt nicht mehr am selben Regler wie die Fluch-Angriffsfläche. Siehe `KAMMER-MESSUNG-2026-08-20.md`, Abschnitt 9. Der folgende Absatz beschreibt den Stand davor.
  **Was sich rechnerisch nicht ändert:** bei `goldUebertragAnteil = 0.5` ist der Bankbetrag derselbe wie vorher (bei ungeradem Gold 1 Gold weniger), und es überleben weiterhin 100 Prozent der Schichtbeute. Gürtel und Kasse sind zwei Hälften desselben Topfes, nicht zwei Abflüsse. Wer will, dass weniger überlebt, ändert nicht die Buchung, sondern das Verhältnis — das ist F20.
  **Was sich für F20 sehr wohl ändert:** der Regler ist jetzt ein Verteiler statt zweier unabhängiger Hähne. Bei 0.5 identisch, bei jedem anderen Wert nicht mehr: 0.3 gab vorher 30 Prozent Gürtel *plus* 30 Prozent Kasse, also 60 Prozent Überleben; jetzt 30 Prozent Gürtel plus 70 Prozent Kasse, also 100 Prozent. Die unter F20 notierte Altlast verschärft sich dadurch: Bankzufluss hoch heißt zwangsläufig Gürtelübertrag runter und damit weniger Angriffsfläche für `Verwaltungsgebühr` und `goldweg`. Das Entkoppeln in zwei Regler ist damit keine Kür mehr, sondern Voraussetzung, sobald F20 den Bankzufluss anfasst. Nicht mit umgesetzt, bewusst — es ist eine Balance-Entscheidung.
  **Vom alten Eintrag bleibt eines falsch und wird hier korrigiert:** die Behauptung, `player.gold` habe keine Ausgabestelle, ist widerlegt (`GEGENPROBE-2026-08-04.md:154`) — die Flüche `Verwaltungsgebühr` und `Goldschwund` ziehen laufend ab, und beide standen schon vor der Analyse im Code. Der ursprüngliche Wortlaut zur Nachvollziehbarkeit:
  > Der Befund ist widerlegt: `player.gold` hat im ganzen Spiel keine Ausgabestelle. Einnahmen kommen aus Loot und Drops (`index.html:3645`, `:4845`), abgezogen wird nur von den zwei Flüchen `Verwaltungsgebühr` (`:2898`) und `goldweg` (`:4778`). Kein Händler, kein Kauf. Kaufkraft ist allein `amt.bankGold` (`buyAusbau`, `buyVermutungen`, `unlockStartFluch`), und die bekommt plankonform 50 Prozent. Der Gürtelübertrag ist keine zweite Buchung von Vermögen, sondern Angriffsfläche für die zwei Flüche. Doppelt ist nur die Buchhaltung, nicht die Wirtschaft. Die vorgeschlagene Gegenvariante `amt.bankGold += player.gold - carryGold` wäre bei geradem Gold ein reiner No-Op und bei ungeradem 1 Gold Unterschied gewesen. Offen bleibt allein die Beschriftung: die Zeile "Beuteanteil nach Abzug" (`:5890`) liest sich als Verlust, und der Spieler erfährt nirgends, dass die Bank etwas bekommen hat. Diese eine Zeile wird zusammen mit F20 erledigt, nicht davor.
* **F20 Ausbau-Kosten. GEMESSEN am 20.08.2026, siehe `KAMMER-MESSUNG-2026-08-20.md`.** Kurzfassung: Bankzufluss im eingeschwungenen Zustand ist die **volle** Schichtbeute, nicht die Hälfte (`carry = beute`), und der Kammerkanal trägt selbst in der pessimistischsten Annahme rund das Sechsfache des Oberweltkanals. Vollausbau damit in rund 2 Schichten statt der 9 bis 23 aus dem Bericht vom 06.08. Empfehlung im Messdokument: **die Preise nicht anheben**, der Hebel läge falsch. Der Absatz unten beschreibt den Stand vor der Messung und bleibt als solcher stehen.
  Vollausbau 3850 Gold gegen etwa 1000 bis 1500 Bankzugang pro Schicht. Der Baum ist ab Schicht 3 bis 4 leer, das erste Jahresgespräch kommt bei Schicht 10. Braucht eine durchgespielte Schicht, nicht Kopfrechnen. **F19 ist am 2026-08-05 doch getrennt erledigt worden**, Buchung und Berichtszeile stehen; hier hängt nichts mehr davon ab. **Altlast vorher lesen, seit F19 verschärft:** `CONFIG.goldUebertragAnteil` steuert mit einem einzigen Regler zwei unabhängige Dinge, den Gürtelübertrag und den Bankzufluss — und seit der F19-Umbuchung sind die beiden nicht mehr nur gekoppelt, sondern gegenläufig: was die Kasse mehr bekommt, fehlt exakt am Gürtel. Getunt werden muss aber nur der Bankzufluss, denn er allein bestimmt, wann der Baum leer ist. Wer den Regler anfasst, ohne ihn vorher zu entkoppeln, verschiebt ungewollt auch die Fluch-Angriffsfläche. Der Kommentar an der Konstante beschreibt bislang nur den Gürtelübertrag und verschweigt die Bank.
...
**Umsetzungsnotizen R8.** Alle zehn Funde von je einem unabhängigen Prüfagenten am Code nachgeprüft. Fünf tragen unverändert (F18, F37, F38, F39, F15), fünf mit Korrektur; drei Teilbehauptungen widerlegt:

* **F64, NPC-Teil widerlegt.** Die drei NPCs hängen seit jeher am selben `currentLevel === 1`-Gate wie Knöterich und werden im Schattenland nie gezeichnet — der Bericht hat das Gate zwei Zeilen über seiner Fundstelle übersehen. Repariert nur die big-Decos: Tönung `#4a1866`/0.72 wie die Baumsilhouetten, gebacken über `TINT_CACHE` (kein `ctx.filter`, Regel 10). Windmühle wandert mit (gleiches Flag, gleicher Mechanismus), hohes Gras bleibt bewusst hell.
* **F71, Zeile-3-Behauptung widerlegt.** „Ministerium für Monsterangelegenheiten" ist kanonische Form 2 aus Weltbibel Kapitel 18 und dort ausdrücklich als CREDITS-Zweitzeile vorgeschrieben. Gestrichen nur Christmass/MilitaryCamp (kein verwendetes Asset, grep leer), Creator-URL `kenmi-art.itch.io` ergänzt.
* **F17, „fehlt in den G3-Notizen" nur halb richtig** (bat steht dort als Schema-Abweichung, nicht als Abnahme-Ausnahme). Entschieden: Kompromiss dokumentieren statt auf `flying_skull` umhängen — das wäre selbst kein voller Rig (attack 2 Frames, cast = attack), und zerberstende Schädel-Bruchstücke widersprächen Name, Drop „Fledermausflügel" (🦇) und Weltbibel „Der Umlauf". Treffer/Tod bleiben über Blitz-Tint und Konfetti-Leiche sichtbar.
* **F15: sechs Manifest-Einträge, nicht drei** (je Dungeon_1 und Dungeon_2). Ursache `TILE_RULE_HINTS` (stuft `/Dungeon_N/` pauschal als 16x16 ein, confidence 1); der fw/fh-Override schlägt die Regel, empirisch belegt.
* **F43: `_rigTable` hatte 24 Einträge, nicht 25**, und ihr fehlten ausgerechnet die 24 Player-Layer- und 3 NPC-Sheets — das Rig, dessen Lücke der Fund als Schaden anführt. `sheet-audit.mjs` liest die `_rigTable` jetzt als anims/fw-fh-Override je Pfad; nach dem Neulauf tragen 52 statt 1 Einträge eine anims-Zuordnung, 11 Raster korrigiert (6 aus F15 plus 5 Schuh-Sheets, deren schmale Silhouette die Inferenz auf 32x64 lockte).
* **F18: der Fixvorschlag des Berichts wäre eine Desktop-Regression gewesen.** `bakeUiSkin()` läuft einmal beim Laden, `body.touch` entsteht erst beim ersten Touch, und dieselben sieben IDs sind auf dem Desktop eckige Gürtelknöpfe. Fix sitzt CSS-seitig in den `body.touch`-Regeln (`border-image-source:none !important`, Rundsprite über `--cfui-round`, `image-rendering:pixelated` für Regel 14). Die Pille `#aktionBtn` (vom Fund nicht genannt, gleicher Defekt) ist mitgeheilt.
* **F37: die Gebäude laufen nicht über `vis()`,** sondern über die y-Band-Schleife der decos — dieselben vier Modulvariablen, aber der Fix sitzt dort: eigener Rand `BIG_PAD = 190` nur für big-Decos statt globaler Aufweitung (die hätte je nach Viewport 15–32 % mehr Zeichenlisten-Einträge gekostet). Markt-Seitenpop (ax:96) und Windmühle (30px) mitgeheilt.
* **F39: Ursache exakt wie gemeldet** (Bits 4/5 von `tileHash` hängen nur von `x&63`/`y&63` ab — 0,0 % gleiche Nachbarn, Periode 64, strukturell, kein Zufall). fmix32-Runde nur in `pickCfTile`; Baumarten und Kammer-Wandprops behalten den rohen Hash. Nachher 25,2/24,5 % gleiche Nachbarn (Soll 25 %), Gegensehen am laufenden Spiel per Screenshot-Vergleich: Streuung statt Gitter. Die falsche Ursachen-Angabe in den G4-Notizen („Tonkontrast") ist als Nachtrag richtiggestellt.
* **F38: nachgemessen per Alpha-Boundingbox** — das Amt lässt 18px links, 8px rechts, 13px unten frei, 5 von 6 Gebäuden betroffen. Der Codekommentar („das Gebäude-Sprite deckt sie ab") war falsch, der Bericht hatte recht; Kommentar richtiggestellt. Bake-Fix (G_WALL backt in Oberwelt und Schattenland als Pfad), Kartenwerte und Kollision unangetastet.
* **F16: reiner Doku-Fix wie vorgesehen** (Abnahme G2 ehrlich formuliert); zusätzlich die nachweislich falsche Pack-Aussage „keine anderen Waffen-Sprites" korrigiert (`Iron_Tools`/`Wooden_Bow` liegen ungenutzt daneben).

Abnahme R8: Syntaxcheck fehlerfrei; 300 Frames mit 25 Zaubern ohne Exception, Konsole leer; Kammer betreten und per Esc verlassen ohne Exception; `knAssertCaps()` true. Frame-Budget (48 Monster, 1280x800, Median aus 9x300 nach 3 Warmläufen, Stash-A/B mit Reload): Minima build-übergreifend gleich (Alt 0,330/0,363 — Neu 0,337/0,341/0,357 ms), die Median-Drift 0,38–0,50 ist Maschinenlast, beide Builds zeigen beide Lastmodi; R8 addiert analytisch ~2–3 µs/Frame. Screenshot-Vergleich gegen den Altstand (F34-Methode) für F37 (identische Kameraposition: Altstand verwirft Amt und Haus komplett, Neustand zeigt beide Dächer am Bildrand), F38/F39 (Dorf: Rechtecke weg, Gitter weg), F64 (Schattenland-Dorf in Silhouetten-Tönung, Fenster bleiben lesbar), F18 (sieben Rundknöpfe rund mit Rundsprite, Desktop unverändert).
