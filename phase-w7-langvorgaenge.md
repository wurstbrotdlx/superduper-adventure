...
**Nummer 2 kollidiert nicht mit Bramsches Frage.** Der Strangschritt sitzt in `npcCycle()`, die Frage in `npcSprechen()` davor. Ein Strangschritt verbraucht einen Zyklus-Tastendruck, nie einen Frage-Tastendruck. Aus dem Boolean `bramscheFragePending` ist der Zähler `bramscheFragen` geworden, fünf Fundstellen *(Korrektur GW: fünf, nicht vier — Deklaration, zwei Stellen in `npcSprechen()`, die `rangSchluessel()`-Ausnahme und `startShift()`)*; `startShift()` setzt ihn auf `1 + (langFertig('anlage3') ? 1 : 0)`, also abgeleitet, kein `amt`-Feld. `rangSchluessel()` aus W6 bleibt unverändert die unendliche Ausnahme.

**Nummer 1 hat absichtlich keinen Bonus.** Kapitel 10 sagt das ausdrücklich zweimal. Die einzige Wirkung ist eine wärmere Begrüßung. Sie steht als **eigene Sprosse ganz oben** in `knBegruessungLine()` und wird nicht angehängt — angehängt hätte sie auf keinem einzigen Rang unter den 44er-Deckel gepasst, die Belohnung wäre gebaut und unerreichbar gewesen. **`anredeAssert()`** prüft ausdrücklich, dass sie auf mindestens einem Rang sichtbar ist *(Korrektur GW26e: hier stand `langAssert()`; die Prüfung liegt in `anredeAssert()`, weiter unten sagt es dieses Dokument selbst richtig)*.
...
* **Die Gießkannen-Wärme verschwindet auf langen Titeln**, weil sie dort nicht unter den 44er-Deckel passt. Der Guard beweist nur, dass sie irgendwo sichtbar ist. *(Korrektur GW8: hier stand „Rang 0 und ab Rang 18". Gemessen ist sie auf **7 von 19** benannten Rängen sichtbar — 3, 6, 9, 11, 12, 13, 15 — und auf **keinem** Rang oberhalb 18. Die Ränge 0 bis 2 sind die Schichten 0 bis 14, in denen der Strang typischerweise abgeschlossen wird: der Regelfall ist, dass der Spieler die einzige Belohnung dieses Strangs nie zu sehen bekommt.)*
...
* Alle **sieben** selbstaufrufenden Guards liefern `true`, die abgefangene `console.error`-Liste ist leer. *(Korrektur GW: `blaetterAssert` mitgezählt; es sind sieben.)*
...
* Sichtprüfung: der **Akten-/Blätter-Reiter** zeigt „LAUFENDE VORGÄNGE" *(Korrektur GW: an vier Stellen dieses Dokuments stand „Kladde-Reiter"; der Block wird in `renderBlaetter()` gerendert und hängt an `tabBlaetter`, nicht an `tabKladde`)* zwischen dem W5-Anschriftenblock und der Blätter-Zählzeile, mit abgeschlossenen Strängen in Grün und laufenden in Sandfarbe.
